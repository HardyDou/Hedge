...
**场景 1：首次启动**
```
双击应用 → 显示主窗口 + 托盘图标 → 输入主密码解锁 → 正常使用
```

**场景 2：快速复制密码（已解锁）**
```
点击托盘图标 → 快捷面板显示密码列表 → 搜索 "github"
→ 鼠标悬停在 GitHub 条目上 → 一侧冒泡显示密码详情
→ 点击密码复制按钮 → 移开鼠标，冒泡消失 → 粘贴密码
```

**场景 3：快速复制密码（未解锁）**
```
点击托盘图标 → 快捷面板显示解锁页面 → 指纹/密码解锁
→ 显示密码列表 → 搜索需要的密码
→ 鼠标悬停在密码条目上 → 一侧冒泡显示密码详情
→ 点击密码复制按钮 → 移开鼠标，冒泡消失 → 粘贴密码
```

**场景 4：后台常驻**
```
关闭主窗口 → 应用缩到托盘 → 随时点击托盘快速访问
```

**场景 5：完全退出**
```
点击托盘图标 → 点击"退出应用"按钮 → 应用完全关闭
```
...
**窗口样式**:
- **风格**：与主应用保持一致（Cupertino 风格）
- **尺寸**：固定宽度和高度，不可调整大小
- **建议尺寸**：宽度 320-360px，高度 480-520px（精致紧凑）
- **外观**：无边框、圆角、阴影效果
- **定位**：从托盘图标位置弹出，智能避免超出屏幕边界
