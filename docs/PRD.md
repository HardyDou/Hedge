...
**状态:** 已完成大部分功能，UI 迁移完成 (Mostly Feature Complete, UI Migration Done)
**最后更新:** 2026-02-27
...
*   **主题管理:** 
    ...
    *   默认为“跟随系统”，可在设置中切换。
*   **添加密码交互 (Cupertino 页面式):** 提供一个独立的“新增条目”页面，采用 Cupertino 风格的表单输入，支持完整录入：标题、账号、密码、URL、备注及附件。
*   **详情页功能**: 
...
*   **设计风格:** 完全遵循 iOS 设计规范 (Apple Human Interface Guidelines)。所有页面采用 Cupertino Widgets，确保原生 iOS 体验。
*   **设计原则:**
    *   采用 iOS 风格的底部 Action Sheet (CupertinoActionSheet/CupertinoModalPopup) 进行选项选择，或使用全屏模态页面。
    *   使用分组样式 (`_buildiOSSection`)、圆角卡片容器 (`CupertinoListTile` 的视觉样式)，分隔线用于列表项。
    *   遵循 iOS 配色方案 (SF Symbols 图标, `#007AFF` 强调色，`CupertinoColors` 调色板)。
    *   统一使用 `SafeArea` 确保各平台显示正确，特别是在自定义导航栏场景。
    *   优化了页面进入和退出动画，确保从左侧进入和退出。 
...
| 功能 | 描述 | 状态 |
|------|------|------|
| **Material → Cupertino UI 迁移** | 将所有 Material 组件替换为 Cupertino 组件，优化页面动画，解决 header 覆盖问题，统一弹窗样式。 | ✅ |
...
| **自动锁定** | 使用 AppLifecycleState 检测后台/前台，切后台立即检查超时并锁定（使用 flutter_app_lock 实现） | ✅ |
