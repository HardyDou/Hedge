...
### Added
- **CLI**: `hedge unlock` - Create session token
- **CLI**: `hedge sync` - WebDAV sync with smart conflict resolution
- **CLI**: `hedge config` - Manage CLI configuration (WebDAV, etc.)
- **CLI**: Native Messaging support for browser extensions

### Fixed
- **CLI**: Remove dead code in config command
- **CLI**: Add Linux old path migration compatibility
- **CLI**: Improve password masking edge cases
- **CLI**: Use PROPFIND for actual remote modification time
